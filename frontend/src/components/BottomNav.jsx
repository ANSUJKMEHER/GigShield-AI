        ...
        {navLinks.map((link) => {
          ...
          return (
            ...
            >
              ...
              >
                <Icon
                  className={`w-6 h-6 ${isActive ? (isAdmin ? "drop-shadow-[0_0_12px_rgba(16,185,129,0.5)]" : "drop-shadow-[0_0_12px_rgba(99,102,241,0.5)]") : ""}`}
                  strokeWidth={isActive ? 2.5 : 2}
                />
              </motion.div>
              <span
                className={`text-[10px] font-bold tracking-wider uppercase transition-all absolute bottom-1 ${isActive ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}
              >
                {link.name}
              </span>
            </Link>
