...
export default function Navbar() {
  ...
  const isLinkActive = (linkPath) => {
    ...
    if (linkPath === '/dashboard') return location.pathname === '/dashboard' && !location.search;
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          <Link to={user ? (isAdmin ? "/admin" : "/dashboard") : "/login"} className="flex items-center gap-2 group">
            <motion.div 
            ...
            <span className="font-bold text-xl sm:text-2xl text-white tracking-tight">GigShield<span className="text-indigo-400">AI</span></span>
          </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              <>
                    ...
                    {workerLinks.map(link => {
                      ...
                      return (
                        <Link 
                          key={link.label} 
                          to={link.path} 
                          className={`px-3.5 py-1.5 text-sm font-bold rounded-full transition-all duration-200 ${active 
               ...
               <div className="text-xs uppercase tracking-widest text-slate-400 font-bold ml-2">Worker Portal</div>
            )}
          </div>
        </div>
      </div>
