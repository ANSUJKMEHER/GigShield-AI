...
import { User, Settings, Save, Loader, Shield, Wallet, Activity, MapPin, Clock, AlertTriangle, IndianRupee } from 'lucide-react';
...
export default function Profile() {
  ...
  const memberSince = user.createdAt ? new Date(user.createdAt).toLocaleDateString(undefined, {year: 'numeric', month: 'long'}) : 'Recently';

  return (
    <motion.div initial="initial" animate="in" exit="out" variants={pageVariants} transition={{ duration: 0.4 }} className="max-w-4xl mx-auto space-y-6 py-6 pb-20 font-sans">
      ...
      <div className="bg-gradient-to-br from-indigo-900/50 via-slate-900/80 to-purple-900/30 backdrop-blur-xl rounded-3xl border border-indigo-500/20 shadow-[0_0_40px_rgba(99,102,241,0.1)] overflow-hidden relative">
         ...
         <div className="p-6 sm:p-8 relative z-10">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
               ...
               <div className="text-center sm:text-left flex-1">
                  <h1 className="text-3xl sm:text-4xl font-black text-white tracking-tight">{user.name}</h1>
                  <p className="text-slate-400 font-medium mt-1 flex items-center justify-center sm:justify-start gap-2">
                     <MapPin className="w-4 h-4 text-indigo-400" /> {user.city || 'Unknown City'} · {user.platform || 'Gig Worker'}
                  </p>
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 border-t border-indigo-500/10">
            {[
               { label: 'Active Plan', value: user.activePlan || 'None', icon: Shield, color: 'text-emerald-400' },
               { label: 'Weekly Premium', value: `₹${user.premium || 0}`, icon: Wallet, color: 'text-indigo-400' },
               { label: 'Total Payouts', value: `₹${user.totalPayouts || 0}`, icon: IndianRupee, color: 'text-purple-400' },
               { label: 'Risk Score', value: `${((user.riskScore || 0.2) * 100).toFixed(0)}%`, icon: Activity, color: 'text-orange-400' },
            ].map((stat, i) => (
               <div key={i} className={`p-4 sm:p-5 flex items-center gap-3 ${i < 3 ? 'border-r border-indigo-500/10' : ''} ${i < 2 ? 'border-b md:border-b-0 border-indigo-500/10' : ''}`}>
        ...
        {hasChangedRisk && (
           <div className="mb-6 bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 flex items-start gap-3 relative z-10">
             ...
             <div>
               ...
               <p className="text-sm text-slate-300">Your base risk parameters have changed. <strong className="text-white">Your premium price is locked for the current week</strong> and will be recalculated at the start of your next billing cycle.</p>
               <button onClick={() => setHasChangedRisk(false)} className="mt-2 text-xs font-bold text-amber-400 hover:text-amber-300 uppercase tracking-widest">Dismiss</button>
             </div>
           </div>
        )}
      ...
      <div className="bg-slate-900/60 backdrop-blur-xl rounded-3xl p-6 border border-red-500/20 shadow-xl relative overflow-hidden">
         ...
         <p className="text-sm text-slate-400 mb-4 relative z-10">Permanently delete your account and all associated data. This action cannot be undone.</p>
