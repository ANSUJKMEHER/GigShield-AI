...
async function checkWeatherAndTriggerClaims() {
  console.log('[Oracle] Running automated background check for parametric disruptions...');
  
  for (const city of CITIES_TO_MONITOR) {
    try {
      const weatherRes = await axios.get(`https://api.open-meteo.com/v1/forecast?latitude=${city.lat}&longitude=${city.lon}&current=precipitation,weather_code`);
      ...
      if (rain > 10) { // Severe rain threshold for auto-trigger
        console.log(`[Oracle Alert] Severe weather detected in ${city.name} (${rain}mm). Initiating Zero-Touch Sequence!`);
        ...
        for (const user of affectedUsers) {
          ...
          const claim = new Claim({
            userId: user._id, triggerEvent: 'Heavy Rain', city: user.city,
            expectedIncome: assumedLoss, actualIncome: 0, loss: assumedLoss, payoutAmount, status: 'Approved',
            fraudChecks: {
              gpsVerified: true, radiusValid: true, workerActive: true, telemetryValid: true, duplicateFree: true, weatherMatch: true, anomalyFree: true
            },
            eventDetails, rejectionReason: ""
          });
          ...
          await user.save();
          console.log(`[Zero-Touch] Auto-credited ₹${payoutAmount} to ${user.name}`);
        }
      }
    } catch (err) {
      console.error(`[Oracle Error] failed to check ${city.name} conditions`, err.message);
    }
  }
}
