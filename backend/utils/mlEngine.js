...
const trainingData = [
  ...
  {
    input: { zoneDanger: 0.1, weatherSeverity: 0.1, pastClaimsFreq: 0.1 },
    output: { premiumScale: 0.1 },
  },
  {
    input: { zoneDanger: 0.1, weatherSeverity: 0.2, pastClaimsFreq: 0.1 },
    output: { premiumScale: 0.12 },
  },

  // Moderate Risk Profiles (Typical User)
  {
    input: { zoneDanger: 0.5, weatherSeverity: 0.1, pastClaimsFreq: 0.1 },
    output: { premiumScale: 0.2 },
  },
  {
    input: { zoneDanger: 0.5, weatherSeverity: 0.5, pastClaimsFreq: 0.1 },
    output: { premiumScale: 0.35 },
  },
  {
    input: { zoneDanger: 0.1, weatherSeverity: 0.1, pastClaimsFreq: 0.5 },
    output: { premiumScale: 0.3 },
  },

  // High Risk Profiles (Flood Prone)
  {
    input: { zoneDanger: 0.9, weatherSeverity: 0.1, pastClaimsFreq: 0.1 },
    output: { premiumScale: 0.4 },
  },
  {
    input: { zoneDanger: 0.9, weatherSeverity: 0.5, pastClaimsFreq: 0.1 },
    output: { premiumScale: 0.6 },
  },

  // Catastrophic / High Liability Profiles (Bad zone + Bad weather + High Claims)
  {
    input: { zoneDanger: 0.9, weatherSeverity: 0.9, pastClaimsFreq: 0.1 },
    output: { premiumScale: 0.85 },
  },
  {
    input: { zoneDanger: 0.9, weatherSeverity: 0.9, pastClaimsFreq: 0.9 },
    output: { premiumScale: 0.95 },
  },
  {
    input: { zoneDanger: 0.5, weatherSeverity: 0.9, pastClaimsFreq: 0.5 },
    output: { premiumScale: 0.7 },
  },
];
...
exports.predictDynamicPremium = (zone, rain_mm = 0, pastClaims = 0) => {
  ...
  if (zoneLower.includes("safe") || zoneLower.includes("high ground"))
    zoneDanger = 0.1;
  if (
    zoneLower.includes("flood") ||
    zoneLower.includes("crime") ||
    zoneLower.includes("slum")
  )
    zoneDanger = 0.9;
