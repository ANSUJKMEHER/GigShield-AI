...
router.post("/simulate-event", async (req, res) => {
  try {
    ...
    const user = await User.findById(userId);
    if (!user || user.activePlan === "None")
      return res.status(400).json({ message: "No active insurance plan" });

    if (user.claimsThisWeek >= user.maxClaimsPerWeek) {
      return res
        .status(400)
        .json({
          message: `Weekly limit of ${user.maxClaimsPerWeek} claims reached.`,
        });
    }
    if (user.coverageRemaining <= 0) {
      return res
        .status(400)
        .json({ message: "Weekly coverage limit exhausted." });
    }
    ...
    if (loss < 100) {
      return res
        .status(400)
        .json({
          message: `Loss of ₹${loss} is below the minimum ₹100 threshold.`,
        });
    }
    ...
    const eventDetails = {
      severity:
        triggerEvent === "Heavy Rain"
          ? `Live Rain: ${liveWeather?.rain || 0}mm`
          : triggerEvent === "High AQI"
            ? "AQI 420"
            : triggerEvent === "Curfew"
              ? "Zone Locked Over 4km"
              : "Server Down 45mins",
    ...
    if (fraudCheck.isFraud) {
      ...
      return res
        .status(400)
        .json({ message: fraudCheck.reason, claim: rejectedClaim });
    }
    ...
    res.json({
      message: `₹${payoutAmount} credited instantly!`,
      payoutAmount,
