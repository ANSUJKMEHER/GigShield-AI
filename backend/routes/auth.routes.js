  ...
  try {
    ...
    res
      ...
      .json({
        ...
        user,
        token,
        premiumSuggestion: riskData.premiumSuggestion,
      });
  } catch (error) {
...
router.get("/user/:id", async (req, res) => {
  try {
    ...
    const claims = await Claim.find({ userId: user._id }).sort({
      createdAt: -1,
    });
...
router.put("/profile/:id", async (req, res) => {
  try {
    const { name, city, platform, zone, workingHoursStart, workingHoursEnd } =
      req.body;
